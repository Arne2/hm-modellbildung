...
\subsubsection{Grafische Überprüfung}
Ein Vergleich der Histrograme in Abbildung \ref{fig:Histogramm_TreppeAb_MA} und Abbildung \ref{fig:Histogramm_TreppeAb_OA}, zeigt, dass die Ausreisser nur einen geringen Unterschied verursachen. In den Abbildungen sind jeweils die eine Normalverteilungskurve dargestellt. Es zeigt sich, dass eine Normalverteilung nicht genau erkenntbar ist.

Beim Betrachten des Quantil-Quantil-Diagramms in Abbildung \ref{fig:QQ_TreppeAb_MA} und \ref{fig:QQ_TreppeAb_OA}, zeigt sich ebenfalls, dass der Unterschied der Gesamten Daten und der bereinigten Daten sehr gering ist und mit bloßem Auge nicht zu erkennen. Aus den beiden Diagrammen wird deutlich, dass sich sich bei beiden Fällen um eine Normalverteilung handelt.
...
Die rechnerische Überprüfung bestätigt die Analyse des Quantil-Quantil-Diagramms. Wie in Tabelle \ref{tab:anpassungstest_TreppeAb_MA} und Tabelle \ref{fig:Histogramm_TreppeAb_OA} zu sehen, liegt in beiden Fällen eine Normalverteilung vor. Es ist zu bemerken, dass ein anpassungstest anhand der Daten mit Ausreissern einen größeren p-Wert erzielt. 
