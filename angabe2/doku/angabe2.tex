...
Abbildung \ref{fig:MQSVIP1000ALL} zeigt die durchschnittliche Warteschlangenlänge. Im Vergleich dazu zeigt Abbildung \ref{fig:MQSVIP1000VGL} die Zusammensetzung der Warteschlangenlänge aus den zwei unterschiedenen Gruppen. Es wird deutlich, dass die Wartezeit der Touristen deutlich länger ist als die der VIP. 

\begin{figure}[!htb]
	\centering
	\begin{minipage}{.5\textwidth}
		\centering
		\includegraphics[width=\textwidth]{abbildungen/1_Phone_VIP/Arrival_1000_Serve_100_dur_1000000_Skip_0/MeanQueueTimeResident.pdf}
		\caption{Durchschnittliche Verweildauer der VIP in der Warteschlange , MeanAr = $1000s$}
		\label{fig:MQTVIP1000}
	\end{minipage}%
	\begin{minipage}{0.5\textwidth}
		\centering
		\includegraphics[width=\textwidth]{abbildungen/1_Phone_VIP/Arrival_1000_Serve_100_dur_1000000_Skip_0/MeanQueueTimeTourist.pdf}
		\caption{Durchschnittliche Verweildauer der Touristen in der Warteschlange , MeanAr = $1000s$}
		\label{fig:MQTTOURI1000}
	\end{minipage}
\end{figure}

Die Abbildungen \ref{fig:MQTVIP1000} und \ref{fig:MQTTOURI1000} zeigen die durchschnittliche Verweildauer in der Warteschlange der VIP und Touristen. Wiederum ist das erwartete Verhalten zu sehen, dass sich die VIP durchschnittlich deutlich kürzere Zeit in der Warteschlange befinden als die Touristen.

\begin{figure}[htpb]
	\centering
	\includegraphics[width=0.8\textwidth]{abbildungen/1_Phone_VIP/Arrival_1000_Serve_100_dur_1000000_Skip_0/MeanSystemSizeAll.pdf}
	\caption{Durchschnittliche Anzahl an Kunden im System, MeanAr = $1000s$}
	\label{fig:MSSVIP1000ALL}
\end{figure} 

\begin{figure}[htpb]
	\centering
	\includegraphics[width=0.8\textwidth]{abbildungen/1_Phone_VIP/Arrival_1000_Serve_100_dur_1000000_Skip_0/MeanSystemSizeTouristAndResident.pdf}
	\caption{Durchschnittliche Anzahl an Kunden im System, MeanAr = $100s$}
	\label{fig:MSSVIP1000VGL}
\end{figure}

Abbildung \ref{fig:MSSVIP1000ALL} zeigt die durchschnittliche Anzahl an Kunden im System insgesamt. Im Vergleich dazu zeigt Abbildung \ref{fig:MSSVIP1000VGL} die Zusammensetzung aus den zwei unterschiedenen Gruppen. Es wird wiederum deutlich, dass sich durchschnittliche mehr Touristen im System befinden, als VIP. Eine Ursache hierfür ist die bereits erwähnte Bevorzugung der VIP. Doch auch der Anteil der VIP an der insgesamten Kundenanzahl (lediglich $0,1$) erklärt diese Abweichung. Es kommen somit deutlich mehr Touristen in den Telefonshop als VIP.
F\\
E\\
H\\
L\\
