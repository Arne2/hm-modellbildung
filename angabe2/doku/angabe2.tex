...
Bei der Betrachtung der in Abbildung \ref{fig:meanQueueSize1000} gezeigte durchschnittliche Warteschlangenlänge fällt auf, dass die Werte wiederum bis zu einer Simulationsdauer von $200000s$ stark variieren schwanken. Anschließend nähern sich die Werte dem dem Erwartungswert von $0,0111111$ an, bleiben jedoch unterhalb. Es handelt sich hierbei um eine Abweichung von ca. $0,0015$, was wiederum durch einen systematischen Fehler, oder ein nicht vollständig eingeschwungenes System erzeugt werden könnte.
...
\paragraph{Erhöhung der Simulationsdauer}
...
\paragraph{Anwendung Gesetzes von Little}
\begin{figure}[htpb]
	\centering
...
\label{Formeln2}
Für die theoretischen Größen und Zeiten des Systems bzw. der Warteschlange werden folgende Formeln genutzt [3]. Dabei steht P0 für die Wahrscheinlichkeit, dass sich keine Kunden im System befinden.
\begin{equation}
...
Da durch zwei Telefone die Service Rate verdoppelt wird, ist in dieser Simulation $\mu$ ungleich $\lambda$. Schließlich ist das erreichen eines Steady State nicht unplausibel.
...
\subsubsection{Vergleich zwei Telefone mit einem}
...
Betrachtet man nun die Auslastung der beiden Telefone in Abbildung \ref{fig:2_Phone_Workload_Vergleich} fällt auf, dass die über die Simulationszeit die Last nicht besonders hoch ist. In diesem Graph ist die Linie bei $2$, da auch die Anzahl der Telefone verdoppelt hat. Nur bei einer Ankunftsrate von $100s$ übersteigt die Auslastung des Systems den Wert $1$. Daher lässt sich schließen, dass in den anderen Szenarien das zweite Telefon überflüssig erscheint.
