...
\begin{figure}[!htb]
	...
	\begin{minipage}{.5\textwidth}
		...
		\caption{Evakuierung eines Raumes mit 2 Türen}
		\label{fig:Eva2TMap}
	\end{minipage}%
	\begin{minipage}{0.5\textwidth}
		\centering
		\includegraphics[width=\textwidth]{abbildungen/Evak4TuerenAnfang.png}
		\caption{Evakuierung eines Raumes mit 4 Türen}
		\label{fig:Eva4TMap}
	\end{minipage}
\end{figure}
...
\begin{figure}[!htb]
	...
	\begin{minipage}{.5\textwidth}
		...
		\caption{Evakuierung eines Raumes mit 2 Türen (Heatmap)}
		\label{fig:Eva2THeatMap}
	\end{minipage}%
	\begin{minipage}{0.5\textwidth}
		\centering
		\includegraphics[width=\textwidth]{abbildungen/Evak4TuerenAnfangHeat.png}
		\caption{Evakuierung eines Raumes mit 4 Türen (Heatmap)}
...
\begin{figure}[!htb]
	...
	\begin{minipage}{.5\textwidth}
		...
		\caption{Evakuierung eines Raumes mit 2 Türen (Ausschnitt)}
		\label{fig:Eva2TMitte}
	\end{minipage}%
	\begin{minipage}{0.5\textwidth}
		\centering
		\includegraphics[width=\textwidth]{abbildungen/Evak4TuerenMitte.png}
		\caption{Evakuierung eines Raumes mit 4 Türen (Ausschnitt)}
		\label{fig:Eva4TMitte}
	\end{minipage}
\end{figure}

Abschließend zeigen die Abbildungen \ref{fig:Eva2TEnde} und \ref{fig:Eva4TEnde} die jeweilige Evakuierungsdauer an. Die entsprechenden Ausschnitte zeigen den Moment, in dem die letzte Person den rot markierten Bereich verlassen hat. Bei der Simulation mit 2 Türen ist das nach ca. $71,88s$, bei der Simulation mit 4 Türen nach ca. $36,98s$. Durch die beiden zusätzlichen Türen verläuft die Evakuierung somit um das $1,94$ fache, also fast das $2$fache schneller. Dies ist ein Indiz für die Richtigkeit der Simulation. Die Abweichung vor allem auf Ungenauigkeiten durch die festen Zellgrößen des Automaten zurückzuführen. Darüber hinaus ist die Implementierung so gestaltet, dass eine Person, welche aktuell nicht ziehen kann, so lange warten muss, bis alle anderen Personen gezogen sind, bis sie wieder ziehen kann. Unabhängig davon, ob die Zelle vor ihr frei ist oder nicht. Durch die beiden zusätzlichen Türen in der Simulation mit 4 Türen tritt dieser Fall viel häufiger auf. Somit warten Personen, obwohl sie theoretisch schon gehen könnten. 
